...
//  MyMemory
//
//  Created by Samuel K on 2018. 3. 16..
...
class DataSync {
  ...
  func downloadBackupData() {
    ...
    get.responseJSON { (res) in
      ...
      //10. 다운로드가 끝났으므로 이후로는 실행하지 않느다.
      ud.setValue(true, forKey: "firstLogin")
    }
    
  }
  
  //Memo 엔터티에 모든 데이터 중, 동기화 되지 않은 것을 찾아 업로드 한다.
  func uploadData() {
    //1. 요청 객체 생성
    ...
    do {
      ...
      //4. 읽어온 결과 집합을 순회하면서 [MemoData]타입으로 변화한다.
  ...
  //인자값으로 입력된 개별 MemoMO객체를 서버에 업로드
  func uploadDatum(_ item: MemoMO, complete:(()->Void)? = nil) {
    ...
    upload.responseJSON { (res) in
      ...
      let resutlCode = jsonObject["result_code"] as! Int
      if resutlCode == 0 {
    ...
    complete?() // 완료처리 클로져 실행
  }
}

//MARK: - DataSync Util 메서드
extension DataSync {
  //String -> Date
